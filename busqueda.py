import numpy as np


def depth_first_search(problema):
    nodo = Nodo(problema.estado_inicial, None, None, 0,
                problema.codigo(problema.estado_inicial))
    if problema.test_objetivo(nodo.estado):
        return nodo
    frontera = [nodo]
    explorados = []
    while len(frontera) != 0:
        nodo = frontera.pop()
        explorados.append(nodo.codigo)
        for accion in problema.acciones_aplicables(nodo.estado):
            hijo = nodo_hijo(problema, nodo, accion)
            if problema.test_objetivo(hijo.estado):
                return hijo
            if hijo.codigo not in explorados:
                frontera.append(hijo)
    return None


def breadth_first_search(problema):
    nodo = Nodo(problema.estado_inicial, None, None, 0,
                problema.codigo(problema.estado_inicial))
    if problema.test_objetivo(nodo.estado):
        return nodo
    frontera = [nodo]
    explorados = []
    while len(frontera) != 0:
        nodo = frontera.pop(0)
        explorados.append(nodo.codigo)
        for accion in problema.acciones_aplicables(nodo.estado):
            hijo = nodo_hijo(problema, nodo, accion)
            if problema.test_objetivo(hijo.estado):
                return hijo
            if hijo.codigo not in explorados:
                frontera.append(hijo)
    return None


def expand(problema, nodo):
    return [nodo_hijo(problema, nodo, accion) for accion in problema.acciones_aplicables(nodo.estado)]


# False represents Cutoff (no solution up to specified depth), None represents no solution found
def depth_limited_search(problema, l):
    nodo = Nodo(problema.estado_inicial, None, None, 0,
                problema.codigo(problema.estado_inicial))
    frontera = [nodo]
    resultado = None
    while len(frontera) != 0:
        nodo = frontera.pop()
        if problema.test_objetivo(nodo.estado):
            return nodo
        if depth(nodo) >= l:
            resultado = False  # cutoff
        elif not is_cycle(nodo):
            for hijo in [nodo_hijo(problema, nodo, accion) for accion in problema.acciones_aplicables(nodo.estado)]:
                frontera.append(hijo)
    return resultado


def iterative_deepening_search(problema, l_max):
    for depth in range(1, l_max+1):  # 0,1,...,l_max.
        result = depth_limited_search(problema, depth)
        if result != False:
            return result


def depth_first_search(problema):
    nodo = Nodo(problema.estado_inicial, None, None, 0,
                problema.codigo(problema.estado_inicial))
    if problema.test_objetivo(nodo.estado):
        return nodo
    frontera = [nodo]
    explorados = []
    while len(frontera) != 0:
        nodo = frontera.pop()
        explorados.append(nodo.codigo)
        for accion in problema.acciones_aplicables(nodo.estado):
            hijo = nodo_hijo(problema, nodo, accion)
            if problema.test_objetivo(hijo.estado):
                return hijo
            if hijo.codigo not in explorados:
                frontera.append(hijo)
    return None


def best_first_search(problema, f=None):
    if f is not None:
        setattr(problema, "costo", f)
    s = problema.estado_inicial
    cod = problema.codigo(s)
    nodo = Nodo(s, None, None, 0, cod)
    frontera = ListaPrioritaria()
    frontera.push(nodo, 0)
    explorados = {cod: 0}
    while not frontera.is_empty():
        nodo = frontera.pop()
        if problema.test_objetivo(nodo.estado):
            return nodo

        for hijo in [nodo_hijo(problema, nodo, accion) for accion in problema.acciones_aplicables(nodo.estado)]:
            s = hijo.estado
            cod = problema.codigo(s)
            c = hijo.costo_camino
            if cod not in explorados.keys() or c < explorados[cod]:
                frontera.push(hijo, c)
                explorados[cod] = c
    return None


class Nodo:

    # Clase para crear los nodos

    def __init__(self, estado, madre, accion, costo_camino, codigo):
        self.estado = estado
        self.madre = madre
        self.accion = accion
        self.costo_camino = costo_camino
        self.codigo = codigo


def nodo_hijo(problema, madre, accion):

    # Función para crear un nuevo nodo
    # Input: problema, que es un objeto de clase ocho_reinas
    #        madre, que es un nodo,
    #        accion, que es una acción que da lugar al estado del nuevo nodo
    # Output: nodo

    estado = problema.transicion(madre.estado, accion)
    costo_camino = madre.costo_camino + problema.costo(madre.estado, accion)
    codigo = problema.codigo(estado)
    return Nodo(estado, madre, accion, costo_camino, codigo)


def find_path(node):
    nodelist = []
    while node.madre is not None:
        nodelist.insert(0, node)
        node = node.madre
    return nodelist


def find_code_path(node):
    codelist = []
    while node.madre is not None:
        codelist.insert(0, node.codigo)
        node = node.madre
    return codelist


def is_cycle(node):
    codes = find_code_path(node)
    return len(set(codes)) != len(codes)


def depth(node):
    if node.madre is None:
        return 0
    else:
        return depth(node.madre) + 1


class ListaPrioritaria():

    def __init__(self):
        self.diccionario = {}

    def __str__(self):
        cadena = '['
        inicial = True
        for costo in self.diccionario:
            elementos = self.diccionario[costo]
            for elemento in elementos:
                if inicial:
                    cadena += '(' + str(elemento) + ',' + str(costo) + ')'
                    inicial = False
                else:
                    cadena += ', (' + str(elemento) + ',' + str(costo) + ')'

        return cadena + ']'

    def push(self, elemento, costo):
        try:
            self.diccionario[costo].append(elemento)
        except:
            self.diccionario[costo] = [elemento]

    def pop(self):
        min_costo = np.min(np.array(list(self.diccionario.keys())))
        candidatos = self.diccionario[min_costo]
        elemento = candidatos.pop()
        if len(candidatos) == 0:
            del self.diccionario[min_costo]
        return elemento

    def is_empty(self):
        return len(self.diccionario) == 0
